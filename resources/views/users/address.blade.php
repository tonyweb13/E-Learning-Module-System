...
<div class="row pl-3 pr-3">
    ...
    <div class="col-md-4 p-1">
        ...
        <input type="text" name="barangay" class="form-control geo-border-primary"  maxlength="100" placeholder="Barangay" value="{{$data->address->barangay ?? ''}}">
    </div>
</div>
<div class="row pl-3 pr-3">
	<div class="col-md-4 p-1">
	    <label>Province</label>
	    <select class="form-control geo-border-primary" id="province" name="province" >
	...
	<div class="col-md-4 p-1">
	    <label>City</label>
	    <select class="form-control geo-border-primary" id="city" name="city"  >
	    	@if($data)
	    	{{$data}}
	    		@if($data->address)
	    		    <option value="{{$data->address->zipcode->cityId->id}}">
    	    		    {{$data->address->zipcode->cityId->city}}
    	    		</option>
	    		@else
	...
	<div class="col-md-4 p-1">
	    <label>Zipcode</label>
	    <select class="form-control geo-border-primary" name="zipcode" id="zipcode" >
	    	@if($data)
	    	    @if($data->address)
	    		    <option value="{{$data->address->zipcode->id}}">
    	    		    {{$data->address->zipcode->zip_code_number}}
    	    		</option>
	    		@else
	    		    <option selected>Select zipcode</option>
	    		@endif
	    	@else
	    		<option selected value="0">Select zip code</option>
	    	@endif
